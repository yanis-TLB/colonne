...
def show_recherche(supabase):
    ...
    try:
        ...
        if 'types_analyse' in df.columns:
            df['types_analyse'] = df['types_analyse'].apply(lambda x: x if isinstance(x, list) else [])
        else:
        ...
        df_f = df.copy()
        if marque_filter != "Toutes": df_f = df_f[df_f['marque'] == marque_filter]
        if usp_filter != "Tous": df_f = df_f[df_f['code_usp'] == usp_filter]
        ...
        if search_text:
            df_f = df_f[df_f['code_colonne'].str.contains(search_text, case=False, na=False) |
        ...
        if not df_f.empty:
            c1, c2 = st.columns(2)
            with c1:
                if st.button("🔄 Réinitialiser"): st.rerun()
            with c2:
                st.download_button("📥 Exporter CSV", data=df_f.to_csv(index=False).encode("utf-8"),
                    file_name="colonnes.csv", mime="text/csv")

            cols_show = [c for c in ['code_colonne','marque','code_usp','longueur_mm',
                'diametre_interne','diametre_grains','numero_serie','statut','types_analyse','commentaire']
                if c in df_f.columns]
            df_aff = df_f[cols_show].copy()
            df_aff['types_analyse'] = df_aff['types_analyse'].apply(lambda x: ", ".join(x) if isinstance(x, list) else "")
            st.dataframe(df_aff.rename(columns={
                'code_colonne':'Code','marque':'Marque','code_usp':'Phase (USP)',
                'longueur_mm':'L (mm)','diametre_interne':'DI (mm)','diametre_grains':'dp (µm)',
                'numero_serie':'N° Série','statut':'Statut','types_analyse':"Types d'analyse",'commentaire':'Commentaire'
            ...
            if code_choisi:
                ...
                if row.get('types_analyse'):
                    ...
                    for t in row['types_analyse']: st.markdown(f"- {t}")
                if row.get('photo_url'): st.image(row['photo_url'], width=200)
                if row.get('commentaire'): st.info(f"💬 {row['commentaire']}")
        else:
